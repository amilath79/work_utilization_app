...
def create_enhanced_features(df):
    try:
        ...
        logger.info(f"✅ Final enhanced DataFrame: {df_enhanced.shape}")
        ...
        if new_features:
            logger.info(f"🆕 {len(new_features)} features:")
            for feat_or in original_cols:
                logger.info(f"  ➤ {feat_or}")
            logger.info(f"🆕 Created {len(new_features)} new features:")
            for feat in new_features:
                logger.info(f"  ➤ {feat}")
        else:
            logger.warning("⚠️ No new features were created.")

        return df_enhanced

    except Exception as e:
        logger.error(f"❌ Error in create_enhanced_features_modular: {e}")
        logger.error(traceback.format_exc())
        return df
...
def train_enhanced_model(work_type_data, work_type):
    ...
    try:
        ...
        if inf_check:
            logger.warning(f"  ⚠️ Infinity values detected in {work_type}, replacing with 0")
            X_selected = X_selected.replace([np.inf, -np.inf], 0)
        
        # Check for NaN values
        nan_check = X_selected.isnull().any().any()
        if nan_check:
            logger.warning(f"  ⚠️ NaN values detected in {work_type}, filling with median")
