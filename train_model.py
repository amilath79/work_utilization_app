...
class WorkforcePredictor:
    ...
    def evaluate_on_holdout(self, model, X_test, y_test, work_type):
        ...
        within_5_pct = np.sum(np.abs((y_test - y_pred) / y_test) <= 0.05) / len(y_test) * 100
        within_10_pct = np.sum(np.abs((y_test - y_pred) / y_test) <= 0.10) / len(y_test) * 100
    ...
    def time_series_cv_evaluation(self, X, y, work_type, n_splits=5):
        ...
        for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
            ...
            model = lgb.LGBMRegressor(**DEFAULT_MODEL_PARAMS)
            model.fit(
                X_train, y_train,
    ...
    def train_model_for_worktype(self, work_type):
        """Train LightGBM model with robust evaluation"""
        # Filter data
        ...
        cv_results, avg_mae = self.time_series_cv_evaluation(
            X_train_cv, y_train_cv, work_type, n_splits=min(5, len(X_train_cv) // 50)
        )
        
        # Train final model on all training data
        final_model = lgb.LGBMRegressor(**DEFAULT_MODEL_PARAMS)
        ...
        production_model = lgb.LGBMRegressor(**DEFAULT_MODEL_PARAMS)
